package nz.coreyh.suppsradar.crawler.infrastructure.persistence.table

import nz.coreyh.suppsradar.crawler.domain.model.CrawlPlatformType
import nz.coreyh.suppsradar.crawler.domain.model.CrawlScopeType
import nz.coreyh.suppsradar.crawler.domain.model.CrawlStatus
import nz.coreyh.suppsradar.persistence.exposed.AuditColumns
import nz.coreyh.suppsradar.persistence.exposed.DeleteColumns
import nz.coreyh.suppsradar.persistence.exposed.createdAt
import nz.coreyh.suppsradar.persistence.exposed.deletedAt
import nz.coreyh.suppsradar.persistence.exposed.updatedAt
import org.jetbrains.exposed.v1.core.dao.id.UuidTable

object CrawlBatchTable : UuidTable("crawl_batch"), CrawlLifecycleColumns, AuditColumns, DeleteColumns {
    val retailerId = uuid("retailer_id")
    val platformType = enumerationByName<CrawlPlatformType>("platform_type", 64)
    val scopeType = enumerationByName<CrawlScopeType>("scope_type", 64)
    val scopeValue = varchar("scope_value", 255).nullable() // null for FULL

    override val status = enumerationByName<CrawlStatus>("status", 64)
    override val startedAt = startedAt()
    override val completedAt = completedAt()
    override val failedAt = failedAt()
    override val createdAt = createdAt()
    override val updatedAt = updatedAt()
    override val deletedAt = deletedAt()
}
